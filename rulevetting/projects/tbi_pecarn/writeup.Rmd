...
`@Xin: I thought we were just going to talk about the rulefit model, which is what I said above, but then we immediately start talking about greedy CART, and we don't provide any figures on it, maybe just reduce the greedy CART down to just a sentence?`
...
the hyperparameter is the maximum length for each rule. In addition, since our rulefit model involves random forest methods to pick the rules, 
the performance may vary even if we choose the same hyperparameters. Therefore, for our rulefit model, we also need to run the models 
...
The greedy rule list model performs quite poorly. For the two datasets excluding the subvariables, the largest mean scores 
...
`@Xin maybe put a few sentences down here about how we tried other classifiers like the greedy rulelist model, but it performed badly`
...
less interpretability than the rule list models in @kuppermann2009. Although we take the rule list model into account
when picking best classifiers, the greedy (CART) rule list model does not perform well. It had lower a sensitivity and was not stable
