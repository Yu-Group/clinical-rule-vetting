...
presenting to ER with head trauma, etc.), to vet Kupperman's
classifiers, we need a dataset that will enable us to develop classifiers with
high predictive accuracy and maximal generalizability. 
...
Kuppermann builds classifiers for two or older versus younger than two.
...
include_graphics('figs/baseline_res.png', dpi=200)
```
...
experiencing clinically-important TBI, the second number is the number of children in this block for whom the next rule will apply, and
the last one represents the proportion. We use the whole cleaned dataset to evaluate the model performance, 
while Kuppermann splits the data into a training set and validation set. The performance of the baseline classifiers
is similar to the result in Kuppermann et al. The baseline classifiers do identify almost all of the cases as 
the sensitivity is 98.9\% and 96.8\% for preverbal children and verbal children, respectively. However, the corresponding specificities are more disappointing:
...
of judgement call, we can improve this performance considerably.

### Procedure and Performance

As discussed in Exploratory Data Analysis, there are arguments for-and-against making
an age-invariant classifier vs. classifiers for different age groups, and making a classifier including and excluding subvariable information.
Taking the cartesian product of these decisions, there are 6 possible datasets on which to develop classifiers
(children <= 2 with/without subvariables, children > 2 with/without subvariables, all children with/without subvariables).
Note that the age-grouped datasets include their relevant age-specific variables, and the
dataset with all children exludes these age-dependent variables, as discussed in EDA.
To find a best classifier, we first determine on which of these datasets does our `Rulefit` model perform the best.
It should be noted the dataset of children <= 2 without subvariables contains too few
TBIs, so we cannot get a reasonable result. Therefore, we will train and evaluate for the other five datasets.

For each possible dataset, we split 20\% of the data into the test data, which will only be used in the model evaluation,
while we use the remaining 80\% to fine-tune the hyperparameters and fit the model. Then, perform a four-fold cross-validation 
to tune the hyperparameters for each model. Since this is an unbalanced dataset, raw classification accuracy will be
...
`Note: how do the next two paragraphs fit into the previous discussion? I'm a bit confused..`
...
```{r rule_result_on_test, echo = FALSE, fig.cap = 'ROC curves of fine-tuned rulefit models on the test data for all five datasets.', out.width = "100%"}
...
(gradient boosting, for example, performs subsampling). The randomness in rule generation
...
classifier from Kupperman, whose results are presented in Baseline Model.
...
if someone has a clinically-important TBI. Comparing to the baseline classifier's variables in 
...
removing the parent variables and giving their children variables a binary classification. 
...
```
Our final model ended up having depth 8 and weight 400, and when applied to the validation dataset, it demonstrated an 
...
```{r perturb_res, fig.cap = 'The sensitivity and specificity are fairly stable, with specificity being slightly more so than sensitivity.', out.width = "100%"}
...
altered mental status, the history of vomiting, hematympanum, severity of injury,
loss of consciousness for more than 5 seconds, and neurological deficit related to the cranial nerve are of vital importance when
classifying the clinically-important TBI. As mentioned in stability section, the results of the baseline classifier
supports this result. Moreover, our classifier can also identify one particular important subvariable of a parent variable.
While our classifier keeps almost 100\% sensitivity, the specificity also improves to about 70\%, 
which is higher than that of the baseline classifier. Moreover, based on results of our data processing and modeling,
we show that splitting children by age is not necessarily a good idea. The dataset is unbalanced, and the the number of preverbal children experiencing
TBI is quite small, which will influence the performance of fitted models.

Additionally, the important features identified by our classifier not only match the baseline model results, but also
...
And when we apply different combinations of perturbations of the data, the sensitivities and specificities
...
less interpretability than the rule list model in the baseline classifier. Although we take the rule list model into account
...
to the data perturbation. We would have tried other type of rule list models to see whether we could get a stable model with 
high sensitivity and specificity, as well as more interpretability. And we may apply other judgement calls to fit the models, such as checking
the importance of including and excluding individual subvariable groups for a particular parent variable.

# Conclusion

In this paper, we utilize the same data as Kuppermann to develop classifiers to identify whether the children 
...
and deal with the parent variable structure as judgement calls. Then we perform EDA and modeling to pick our best classifier, which
is built on age-invariant data and including the subvariables. Its sensitivity and specificity outperforms the baseline models derived by
...
loss of consciousness for more than 5 seconds, and cranial nerve neurological deficits,
...
when we perform data perturbations. And our model can be generalized to the future data based on our assessment to the randomness 
