...
* Drop and impute based on seizure variables
  ...
  + Judgment Call [`Seiz`]: Drop missing *SeizLen*, impute missing *Seiz* from *SeizLen*, drop missing *Seiz*, and ignore *SeizOccur* column
...
* Drop and impute based on loss of consciousness variables
  + *LOCSeparate*, *LocLen*
  + Judgment Call [`LOC`]: recode 'suspected' as 'yes' in *LOCSeparate*, and drop observations with missing *LOCSeparate* and *LOCLen*
    + Perturbation 1: same as [`AMS`] perturbation
    + Perturbation 2: same as [`AMS`] judgment call
...
### An introduction to and an illustration of `rulefit`
...
```{r basic_ROC, echo = FALSE, fig.width=6, fig.height=4, fig.cap="Sensitivity-specificity curve for two basic rulefit models with maximal and minimal judgment calls."}
...
threshold value; since we, however, care much more about false negatives than positives, we consequently value
...
`TODO: check this text if it still fits:`
...
And we include two different judgment calls: whether include or exclude all subvariables.
...
In addition, when we perform the classification, we also need to pick a threshold of the probability, to determine the predicted labels.
For the age-invariant classifier, based on our discussion of sensitivity and stability, the threshold is set to $1.3 \times 10^{-5}$, 
which persform well not only on training and validation data, but also on test data. For test data classification,
the sensitivity is 97.9\% and the specificity is 70.4\%.
...
`What does randomness mean in the data? Measurement randomness, genetics/confounding factors that make TBIs more or less likely given some covariates, etc.`

`Underneath the model is LASSO regression, a fit on probability space, so describe the randomness mechanism here. Randomness is that the model is greedy, so you don't know where it will end up. Also, what LASSO throws away is some randomness..`  

`Justification for this randomness: throwing away/restricting the feature spaceis just like what a doctor would do in diagnosing a TBI: you already have some  heuristics to follow, so you would look for those first before looking for less important variables. Same thing that our model is doing: looking for more important features.`  


The randomness in the data can be divided into two categories: *intrinsic* and *measurement-related*.

Under intrinsic we can count the natural variation in people's physiologies, where the a
kind of an injury might lead to traumatic brain injury in one child, but not in another, as well as
the minute differences in the injury mechanics, etc. These might spare one and not another from developing
a traumatic brain injury. Even if our rules somehow covered all the potential avenues to *ciTBI*, these
would, together with potential additional confounding factors, still conspire to make the prediction a probabilistic 
...
These containing a degree of random "noise" should be obvious. As Dr. Inglis pointed
...
such as the baseline one. As shown in [An introduction to and an illustration of `rulefit`], the 
...
is somewhat ameliorated by the large number of trees we're usually generating, 
...
- Rules/variable importance stability:
  - Model perturbations: Compare rules isolated by Ian's RF, baseline classifier, and Xin's best model to assess stability across models


One form of perturbation in our analysis will be a change of model; by comparing the importance of different 
rules across similar models we can get a sense of how consistently the effect appears and thus how stable it is. 
The models to which we will compare our best classifier are both tree-based, including an age-invariant random 
forest classifier and the baseline age-splitting classifier that Kupperman et al. used.

For our random forest, the judgment calls used to obtain the dataset were those that flattened the data, 
...
do not weight our model in order to favor true positives over true negatives, any random forest will predict
that no child has a TBI; thus we add a weight factor to our "1" outcome in order to increase our model's sensitivity.
...
these parameters, testing a total of 77 models for the ideal model that maximizes 2(Sens) + Spec, Spec>0.5, and generated
ROC curves for each, which can be seen in tbi_rf.pdf. 
...
impressive sensitivity of 0.971 and specificity of 0.617- good given the relative costs of letting a TBI go undiagnosed
versus performing an unneeded CT test. In terms of importance, basilar hemorrhage was by far the most important predictor,
which is entirely in keeping with its high prevalence in our rule-fitting model. Other important factors included ActNorm,
ClavPar (scalp trauma), and AMSSlow- all of which show up somewhere in rules as well. Somewhat curiously, no vomiting-related
values were in the top 10 (highest was VomitLast=1 which was 13th most important) despite its high prevalence in our rule-fitting
classifier; this may be a consequence of the flattening of the vomit variable, leaving us with many minor effects instead of the 
one overall binary indicator of whether or not the child was vomiting.

  - Similar for data perturbations since baseline is different from ours, and Andrej's is different data
  
  This will justify the stability of the rules our best classifier isolated
  
- Stability of performance across data perturbations
  - perturb the data by selecting some of the judgment calls, but only perturb test data and see how the model fares (the classification errors)
  
  judgment calls to turn on/off: `SFxPalp`, `Seiz` (change from recoding to not recoding), `ActNorm` recoding (change it up), `Vomit` (either union and impute or not)
  (if we have enough time, talk more about subsampling, otherwise just work on this)
