...
presenting to the ER with head trauma, etc.), we need a dataset that will enable us to develop classifiers with
high predictive accuracy and maximal generalizability. Then, we may compare this new classifier
to previous classifiers to get a sense of how accurate or generalizable they are.
For instance, we would need need a dataset containing covariates that are both relevant to the presence
of TBIs and can be measured at the time a child is ushered into the ER. From common
sense, it seems the injury mechanism (and a rating of
severity), any symptoms such as headache or amnesia, and any results of a physical
examination that may indicate neurological deficits or skull injury are important. For both
