...
for justification while also answering Q8 about any randomness assumptions. 
...
There are three possible splitting methods in age groups: perverbal children, verbal children, and all children.
And we include two different judgement calls: whether include or exclude all subvariables.
Although there are six possible datasets, the dataset of perverbal children excluding subvariables contain too few
...
The performance of rulefit model on test data is in Figure \ref{fig:rule_result_on_test}. We can find that the performance for age-invariant group is quite well. As the sensitivity achieves almost 100\%, the sepecificity is larger than 70\%. And including subvariables
