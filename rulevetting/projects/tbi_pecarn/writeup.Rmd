...
`@Xin: I thought we were just going to talk about the `Rulefit` model, which is what I said above, but then we immediately start talking about greedy CART, and we don't provide any figures on it, maybe just reduce the greedy CART down to just a sentence? -Jimmy`
...
the hyperparameter is the maximum length for each rule. In addition, since our  model involves random forest methods to pick the rules, 
