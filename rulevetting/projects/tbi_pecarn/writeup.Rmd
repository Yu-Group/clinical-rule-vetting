...
### An introduction to and an ilustration of `rulefit`
...
We also added a linear term of all the original features $\vec{\alpha_0}$, which will come into play especially with 
...
As shown in the next chapters, with some careful construction of the models and selection
...
Underneath the model is LASSO regression, a fit on probability space, so describe the randomness mechanism here. Randomness is that the model is greedy,
so you don't know where it will end up. Also, what LASSO throws away is some randomness..

Justification for this randomness: throwing away/restricting the feature space
is just like what a doctor would do in diagnosing a TBI: you already have some
heuristics to follow, so you would look for those first before looking for less important variables.
Same thing that our model is doing: looking for more important features.
