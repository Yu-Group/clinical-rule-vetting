...
We also evaluate our three models on the test set. The results are reported in Table XX. We see that all three models achieve >.98 sensitivity on the test set with over .74 specificity. These results are comparable with their counterparts on the tuning set (and are even slightly better), which indicates that our models do not suffer from overfitting and  can generalize well to new data from the same population.
