...
Prior to modeling, we split our data into *training* (60% of data), *tuning* (20% of data), and *testing* (20% of data) sets. The training set will be used to construct our candidate models. The tuning set will be used to select an optimal model, and the testing set will be used to corroborate the performance of our best model.
...
```{r roc-curves, fig.width=6.3, fig.height=6, fig.cap="ROC curves for the three candidate models. The black dot represents the CDR from Leonard et al.", out.width='85%'}
...
dc_ada <- melt_ada %>%
  ...
  scale_fill_manual("Accuracy Metric", values = c("#003262", "#00A598")) + 
  theme_classic() +
  geom_text(aes(label = round(value, 2)), vjust = -0.4, position = position_dodge(0.9)) + 
...
We see that for all three models, the sensitivity are higher ($>0.98$) if we use group all control uniformly. This makes sense since the prediction accuracy is likely to increase if we have a larger training set. When comparing one control at a time, all three models can achieve sensitivity above 97% with about 70% specificity except for the decision tree with MOI control. 

In general, this stability check demonstrates that our models are robust to the choice of control group in almost every case. The only counterexample here is probably due to the discrete nature of decision tree, in which the confidence output (the confidence of the model that a patient has a cervical injury) takes its value in a finite set whose cardinality is the number of leaves. The performance of Adaboost and Logistic are identical in all cases. This suggests that Adaboost and Logistic can converge to the same model after training in our specific problem.


# Test set performance


As a final step, we evaluate the performance of AdaBoost on the test dataset, the results of which are reported in Table \blue{\ref{tab:test-res}}. We see that AdaBoost achieves sensitivity above 98% on the test set with over 76% specificity. This results is comparable with its counterpart on the tuning set which indicates that our models do not suffer from overfitting and generalize well to new data from the same population.
...
test_results %>% knitr::kable(caption = "\\label{tab:test-res}Performance of AdaBoost on test data.")
```
