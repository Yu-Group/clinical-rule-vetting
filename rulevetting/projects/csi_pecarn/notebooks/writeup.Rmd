...
To further explore how the features are distributed, we examine the frequency of "yes" responses for the feature space. The ten features with the highest frequency of "yes" values are shown in Figure \blue{\ref{fig:feat-freq}}.


```{r feat-freq, fig.cap="Top ten features with highest \'yes\' (1) responses.", out.width='90%', fig.width=9, fig.height=5}
features <- all %>% select(-outcome) %>% select(!contains("2"))
freq1 <- sort(apply(features, 2, mean), dec = TRUE)
top <- freq1[1:8]
names(top)[c(2, 3)] <- c("CSImmob", "LimitedRM")
...
The `PtCompPain` and `CSImmob` features have the two highest frequencies of "yes" responses. They represent whether the patient complained of pain and whether cervical spine precautions were implemented prior to arrival, respectively. These high percentages make sense because patients usually will not report to the hospital if they do not believe they are in pain. Moreover, emergency medical service technicians will likely implement CSI precautions on most patients (such as a brace or collar) since there is really no harm in doing so.
...
\textcolor{red}{\textbf{ZZQ Licong, you will need to write about this since you did it. Or you can delineate the method for me.}}
