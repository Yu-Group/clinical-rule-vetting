...
class Dataset(DatasetTemplate):
    ...
    def preprocess_data(self, cleaned_data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df = working_helper.extract_numeric_data(df,categorical_covariates=categorical_covariates)
        
        df = working_helper.build_binary_covariates(df)
    ...
    def impute_data(self, preprocessed_data: pd.DataFrame, keep_na=False, **kwargs) -> pd.DataFrame:
        ...
        if not keep_na:
            ...
            df = working_helper.impute_missing_binary(df, n=kwargs['frac_missing_allowed']) 
