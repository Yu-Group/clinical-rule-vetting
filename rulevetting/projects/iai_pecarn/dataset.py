...
class Dataset(DatasetTemplate):
    def clean_data(
        self, data_path: str = rulevetting.DATA_PATH, **kwargs
    ) -> pd.DataFrame:
        ...
        fnames = sorted(
            [
                ...
                if "csv" in fname and not "formats" in fname and not "form6" in fname
            ]
        )  # remove outcome
        ...
        for i, fname in tqdm(enumerate(fnames_small)):
            ...
            df_features = (
                ...
                .combine_first(df2.set_index("id"))
                .reset_index()
            )
    ...
    def preprocess_data(self, cleaned_data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        if not kwargs["prop"]:
            ...
            df = cleaned_data.dropna(
                axis=1,
                thresh=(1 - kwargs["frac_missing_allowed"]) * cleaned_data.shape[0],
            )

            # impute missing values
            # fill in values for some vars from unknown -> None
            df.loc[
                df["AbdomenTender"].isin(["no", "unknown"]), "AbdTenderDegree"
            ] = "None"

            # pandas impute missing values with median
            df = df.fillna(df.median())
            df.GCSScore = df.GCSScore.fillna(df.GCSScore.median(numeric_only=True))
        else:
            df = cleaned_data
            contin_cols = df.dtypes.astype(str).str.contains("float")
            df.loc[:, contin_cols] = df.loc[:, contin_cols].fillna(
    ...
    def extract_features(
        self, preprocessed_data: pd.DataFrame, **kwargs
    ) -> pd.DataFrame:
        ...
        # narrow to good keys
        feat_names = [k for k in df.keys() if not "iai" in k.lower()]  # features to use
        ...
        feats = rulevetting.api.util.get_feat_names_from_base_feats(
            feat_names, base_feat_names=base_feat_names
        ) + ["outcome"]
...
if __name__ == "__main__":
    ...
    df_full = dset.export_full_dataframe()
    print(
        "successfuly processed data\nshapes:",
        df_full.shape,
        "\nfeatures:",
        list(df_full.columns),
    )
    df_full.to_pickle("iai-i.pkl")
