...
class DatasetTemplate:
    ...
    def get_data(self, save_csvs: bool = False,
                 ...
                 resampling: int = 0) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        if resampling == 0:
            ...
            LOC = oj(PROCESSED_PATH, "resampled_data", str(resampling))

        if load_csvs:
            return tuple([pd.read_csv(oj(LOC, s), index_col=0)
        ...
        if not run_perturbations:
            if resampling == 0:
                ...
                np.random.seed(resampling)
                random.seed(resampling)
                extracted_features = extracted_features.sample(frac = 1)
        ...
        if save_csvs:
            ...
            if not run_perturbations:
                ...
                for df, fname in zip([df_train, df_tune, df_test],
                    ...
                    df.loc[:, meta_keys].to_csv(oj(LOC, f'meta_{fname}'))
                    df.drop(columns=meta_keys).to_csv(oj(LOC, fname))
